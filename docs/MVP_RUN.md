...
RAG nằm độc lập trong `libs/rag`. Agent chỉ gọi interface của RAG, nên
...
Đặt PDF trong `libs/rag/data/papers`, rồi chạy:
...
```bash
PYTHONPATH=apps/api .venv/bin/uvicorn server:app \
...
Nếu backend ở URL khác, tạo `apps/web/.env.local`:
