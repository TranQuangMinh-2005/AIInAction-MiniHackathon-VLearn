...
def search_online(state: AgentState) -> AgentState:
    ...
    try:
        if paper_source:
            ...
            if not context:
                # A-08 — arXiv rỗng → hồi sinh web search (Tavily/DDG)
                web_context, web_citations, web_details = query_web(
                    search_query or question
                )
                if web_context:
                    return {
                        **state,
                        "web_search_result": web_context,
                        "citations": web_citations,
                        "citation_details": web_details,
                    }
                return {
                    **state,
                    "web_search_result": (
