...
def refuse_off_topic(state: AgentState) -> AgentState:
    """A-01 — Node từ chối lịch sự khi intent=off_topic (deterministic, 0 LLM call)."""
    return {
        **state,
        "final_answer": (
            "Mình là trợ lý học tập của khóa học **AI Thực Chiến** — mình chỉ hỗ trợ "
            "câu hỏi liên quan đến nội dung bài giảng, bài tập và kiến thức AI/ML. "
            "Câu hỏi này nằm ngoài phạm vi đó nên mình không thể trả lời. "
            "Bạn muốn hỏi gì về slide đang xem không?"
        ),
        "citations": [],
        "citation_details": [],
        "needs_web_search": False,
        # A-05 — envelope tối thiểu cho mọi turn (kể cả từ chối)
        "move": "validate",
        "misconceptions": [],
...
def generate_answer(state: AgentState) -> AgentState:
    ...
    # A-01 fix: research path (web_search → generate_answer) không set
    # slide_search_result → defensive `or ""` tránh `.strip()` trên None.
    slide_result = state.get("slide_search_result") or ""
